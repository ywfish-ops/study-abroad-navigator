...
1. 项目骨架 + 部署流程（进行中）
2. 模块 0：选校探索（学校分类检索）
   - 定位：初选阶段的浏览入口，先于其他功能模块开发
   - 筛选维度：国家（多选）/ 难度档位 S/A/B/C / 专业方向 / 4 年预算范围（实时汇率换算）/ 城市规模 / 学制年数
   - 展示：每所学校一张卡片（校名中英文、国家、QS 排名、4 年预算估算、难度标签）
   - 操作：一键将学校加入财务沙盘对比
   - 数据来源：直接读取 schools.json
3. 院校数据整理（schools.json）
4. 模块 1A：学术定位测试
5. 模块 1C：财务沙盘
6. 模块 1B：联申定位器
7. 移动端适配打磨
...
- **定位**：首页的核心内容，不是独立页面；初选阶段的浏览入口，先于其他功能模块开发

### 首页布局

| 区域 | 内容 |
|------|------|
| 顶部筛选栏 | 国家（多选）/ 难度档位 S/A/B/C / 专业方向 / 4 年预算滑块（实时汇率换算）/ 学制年数 |
| 主体卡片网格 | 每张卡片：校名（中英文）、国旗、QS 排名、4 年预算估算、难度标签、「加入对比」按钮 |
| 底部导航 | 探索 / 测试 / 沙盘 / 我的 |

### 其他规格

- **筛选维度**：国家（多选）/ 难度档位 S/A/B/C / 专业方向 / 4 年预算范围（实时汇率换算）/ 城市规模 / 学制年数
- **登录策略**：无需登录即可浏览；点击「加入对比」后提示注册以保存数据
- **数据来源**：直接读取 `schools.json`，不请求外部接口
- **对应文件**：`explorer.js`（待创建）；开发前先在 `/docs/module-specs/` 补充 `0-explorer.md`
...
OpenClaw 提取官网 ──→ batch_b（专业、语言要求、截止日期、奖学金）
Claude.ai 手动整理 ───→ batch_c（就业城市、行业、校友、校园风格）
基于 batch_a/b 计算 ──→ finance_sandbox（4年总成本、生活费、保险）
```

### 当 Leo 给你 OpenClaw 输出的数据时

Leo 会把 OpenClaw 返回的 JSON 直接粘贴给你（已经人工核查过），你需要：
...
- ❌ 不要从互联网搜索数据来填充——数据源由 OpenClaw 和 College Scorecard 负责
- ❌ 不要改变 schools.json 的顶层结构（_schema、schools 数组）
- ❌ 不要删除已有学校的数据，即使 OpenClaw 新数据中没有那所学校
